# 가격 정책 v1

이 문서는 현재 코드 기준 가격 정책의 기준선이다. 리팩터링 중에는 이 문서의 금액이 바뀌지 않아야 하며, 금액 변경은 별도 정책 변경으로 다룬다.

## 1. 공통 원칙

1. 숫자로 계산 가능한 품목은 제품금액에 포함한다.
2. 상담 안내 품목은 제품금액과 예상 결제금액에서 제외한다.
3. 상담 안내 품목은 화면에는 `상담 안내`, payload에는 `null + displayPriceLabel` 형태로 표현한다.
4. 부가세(`vat`)는 현재 모든 페이지에서 `0`으로 계산한다.
5. 최종 예상 결제금액은 `제품금액 + 숫자로 확정된 배송/시공비`이다.
6. 제품 금액 반올림은 모든 품목에서 `10원 단위 올림(ceil)`을 사용한다.

## 2. pricingRule 스키마

모든 금액은 `pricingRule`를 기준으로 정의한다. 새 품목은 아래 스키마만 사용한다.

### 2.1 공통 필드

| 필드 | 설명 | 예시 |
| --- | --- | --- |
| `type` | 가격 계산 방식 | `fixed`, `area`, `meter`, `perHole`, `tieredBySize`, `tieredByWidth`, `tieredByHeight`, `free`, `consult` |
| `value` | 고정 단가 또는 기본 단가 | `47000` |
| `unit` | 단가 기준 단위 | `m2`, `meter`, `hole`, `item`, `piece` |
| `label` | 표시용 문구 | `가용높이 내 무료` |
| `tiers` | 크기/치수별 티어 목록 | `[ { maxWidth, maxLength, price } ]` |
| `priceByTierKey` | 티어 키별 가격 맵 | `{ "lte_400": 9000, "lte_600": 11000 }` |
| `priceByCategory` | 카테고리별 가격 맵 | `{ "LPM": 22000, "PP": 20000 }` |
| `availabilityRule` | 상담 전환/가용성 조건 | `ok`, `consult`, `conditional` |

### 2.2 권장 예시

```js
pricingRule: {
  type: "area",
  value: 47000,
  unit: "m2",
}
```

```js
pricingRule: {
  type: "tieredBySize",
  tiers: [
    { maxWidth: 300, maxLength: 800, price: 35000 },
    { maxWidth: 400, maxLength: 800, price: 40000 },
  ],
}
```

### 2.3 운영 원칙

1. 신규 품목은 `pricingRule`를 반드시 포함한다.
2. `pricingRule`가 있으면 계산은 `pricingRule`를 우선 사용한다.
3. 레거시 가격 필드는 사용하지 않는다.
4. 가격 표시는 계산 스키마와 동일한 원본에서 생성한다.

### 2.4 프로모션 할인 스키마

프로모션 설정은 `assets/js/data/promotion-data.js`의 `ORDER_PROMOTION_CONFIG`를 사용한다.

| 필드 | 설명 | 예시 |
| --- | --- | --- |
| `enabled` | 전체 프로모션 활성화 | `true` |
| `rules[].id` | 룰 식별자 | `monthly_lpm_walnut_10` |
| `rules[].enabled` | 개별 룰 활성화 | `true` |
| `rules[].pages` | 대상 페이지 | `["board", "door", "top", "system"]` |
| `rules[].targetTypes` | 대상 타입 | `["material"]`, `["furniture"]` |
| `rules[].materialIds` | 자재 ID 필터 | `["lpm_natural_walnut"]` |
| `rules[].addonIds` | 가구 ID 필터 | `["GFD-2"]` |
| `rules[].discountRate` | 할인율(0~1) | `0.1` |
| `rules[].startsOn` | 시작일(KST, YYYY-MM-DD) | `"2026-04-01"` |
| `rules[].endsOn` | 종료일(KST, YYYY-MM-DD) | `"2026-04-30"` |

규칙:
1. 날짜는 KST 00:00~23:59:59 기준으로 해석한다.
2. 조건이 여러 개 매칭되면 가장 큰 `discountRate` 1개만 적용한다.
3. 현재 할인은 자재(`material`)와 시스템 가구(`furniture`)에만 적용한다.

## 3. 합판

합판 자재 단가는 `assets/js/data/board-data.js`에 둔다.

| 카테고리 | 단가 |
| --- | ---: |
| LPM | 47,000원/㎡ |
| PP | 45,000원/㎡ |

계산식:

```text
면적㎡ = (폭mm / 1000) * (길이mm / 1000)
품목금액 = 면적㎡ * ㎡단가 * 수량
총액 = ceil(품목금액 + 옵션금액 + 가공서비스금액, 10원)
```

규격 한계를 넘는 경우 상담 안내로 처리한다.

## 4. 상판

상판 가격 정책은 `assets/js/data/top-data.js`의 `TOP_PRICING_POLICY`에 둔다.

| 카테고리 | 기준 단가 |
| --- | ---: |
| 인조대리석 | 147,000원/m |
| 하이막스 | 210,000원/m |

기준 조건:

| 항목 | 기준 |
| --- | ---: |
| 기준 두께 | 12T |
| 자동 계산 가능 깊이 | 760mm 이하 |
| 금액 올림 단위 | 10원 |
| ㄱ자/역ㄱ자 추가비 | 30,000원 |
| 뒷턱 가용 높이 | 760mm - 상판 깊이 |

과금 길이:

| 형태 | 과금 길이 |
| --- | --- |
| ㅡ자 | `길이` |
| ㄱ자/역ㄱ자 | `길이1 + 길이2 - 깊이` |
| ㄷ자 | `길이1 + 길이2 + 길이3 - 깊이 * 2` |

계산식:

```text
품목금액 = ceil((과금길이mm / 1000) * 카테고리 m단가, 10원)
가공서비스금액 = 형태 추가비 + 선택 가공비
총액 = ceil(품목금액 + 옵션금액 + 가공서비스금액, 10원)
```

12T가 아니거나, 깊이 760mm 초과, 길이 한계 초과, 단가 없는 카테고리는 상담 안내로 처리한다.

## 5. 도어

도어 가격 티어는 `assets/js/data/door-data.js`의 `DOOR_PRICE_TIERS_BY_CATEGORY`에 둔다.

| 카테고리 | 300×800 이하 | 400×800 이하 | 600×800 이하 |
| --- | ---: | ---: | ---: |
| LX SMR PET | 40,000원 | 45,000원 | 50,000원 |
| LX Texture PET | 40,000원 | 45,000원 | 50,000원 |
| Hansol PET | 40,000원 | 45,000원 | 50,000원 |
| LX PET | 35,000원 | 40,000원 | 45,000원 |
| Original PET | 35,000원 | 40,000원 | 45,000원 |
| LPM | 35,000원 | 40,000원 | 45,000원 |

계산식:

```text
품목금액 = 규격 티어 가격 * 수량
경첩가공비 = 유효 경첩 홀 수 * 1,500원 * 수량
총액 = ceil(품목금액 + 옵션금액 + 가공서비스금액 + 경첩가공비, 10원)
```

600×800mm를 넘는 규격은 상담 안내로 처리한다.

## 6. 시스템

시스템 가격 정책은 `assets/js/data/system-data.js`의 선반/포스트바 티어에 둔다.
시스템 가구(addon)는 `pricingRule.type = "tieredByWidth"`와 `priceByTierKey`를 사용하며, 실제 키는 `w600__lpm_basic`처럼 `티어키__자재ID` 조합으로 관리한다.

일반 선반:

| 폭 | LPM | PP |
| --- | ---: | ---: |
| 400mm 이하 | 9,000원 | 8,000원 |
| 600mm 이하 | 11,000원 | 10,000원 |
| 800mm 이하 | 14,000원 | 12,000원 |

코너 선반:

| 유형 | LPM | PP |
| --- | ---: | ---: |
| 코너 표준 | 22,000원 | 21,000원 |

포스트바:

| 유형 | 2100 이하 | 2300 이하 | 2500 이하 |
| --- | ---: | ---: | ---: |
| 기본 포스트바 | 17,400원 | 18,000원 | 19,300원 |
| 코너 포스트바 | 8,000원 | 8,000원 | 8,000원 |

계산식:

```text
선반금액 = 선반 티어 단가 * 선반 수 * 수량
선반 옵션금액 = 선택 옵션 단가 합계 * 수량
포스트바금액 = 높이 티어 단가 * 개수 * 수량
총액 = ceil(선반금액 + 선반 옵션금액 + 포스트바금액, 10원)
```

선반 폭 800mm 초과, 선반 길이 한계 초과, 커스텀 가공, 포스트바 높이 2500mm 초과는 상담 안내로 처리한다.

## 7. 배송/시공

배송/시공 정책은 `assets/js/data/fulfillment-policy-data.js`에 둔다.

| 페이지 | 배송 | 시공 |
| --- | --- | --- |
| 상판 | 상담 안내 | 50,000원 |
| 합판 | 상담 안내 | 상담 안내 |
| 도어 | 상담 안내 | 5개까지 50,000원, 추가 1개당 10,000원 |
| 시스템 | 상담 안내 | 포스트바 2개 50,000원, 3개 70,000원, 4개 90,000원, 그 외 섹션 길이 기준 |

현재 배송은 4품목 모두 상담 안내로 통일한다. 도어와 시스템의 기존 배송 계산식은 정책 `mode`가 `groupedFee` 또는 `compositeGroupedFee`로 바뀌는 시점에 다시 활성화한다.

## 8. 검증 기준

대표 케이스는 `scripts/verify-pricing-policy.mjs`로 검증한다.

현재 검증 범위:

1. 합판 LPM/PP 면적 단가
2. 상판 카테고리 단가, 과금 길이, 형태 추가비, 기준 두께/깊이
3. 도어 규격 티어와 비규격 상담 전환
4. 시스템 선반/포스트바 티어, 선반 옵션, 상담 전환
5. 상담 품목 payload 가격 표현(`null + displayPriceLabel`)
6. 페이지별 배송/시공 정책 주요 금액
