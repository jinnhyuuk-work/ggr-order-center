...
- 목적: 시스템 페이지 추가 기능 2건(타입/설치공간 제약, 모듈 모달 정면 미리보기)의 구현 기준을 먼저 고정한다.
...
1. 타입 선택(`ㅡ자/ㄱ자/역ㄱ자/ㄷ자/ㅁ자`) + 설치공간별 사이즈 입력 + 가용범위 제약
...
| 체크포인트 | 기능 A: 타입/설치공간 제약 | 기능 B: 모달 정면 미리보기 | 반영 상태 |
|---|---|---|---|
...
## 5. 기능 A 스펙: 타입 + 설치공간 사이즈 + 가용범위 제약

## 5.1 사용자 시나리오

1. 사용자가 시스템 타입(ㅡ/ㄱ/역ㄱ/ㄷ/ㅁ)을 선택한다.
2. 타입에 맞는 설치공간별 사이즈를 입력한다.
3. 시스템은 입력값의 가용범위를 검사한다.
4. 탑뷰 미리보기에서 타입 윤곽과 설치공간 경계/치수를 보여준다.
...
## 5.4 타입별 설치공간 구성 정의 (초안)

아래는 UI/검증/미리보기가 공통으로 참조할 타입 정의 테이블이다.

```js
const LAYOUT_TYPE_DEFS = {
  i_single: { label: "ㅡ자", sectionCount: 1, closedLoop: false },
  l_shape:  { label: "ㄱ자", sectionCount: 2, closedLoop: false },
  rl_shape: { label: "역ㄱ자", sectionCount: 2, closedLoop: false },
...
1. 타입 미선택 시 `invalid`
2. 타입별 required 설치공간 길이 미입력 시 `invalid`
...
1. 한 설치공간 길이가 `8000mm 이상`이면 `consult`
...
1. 가장 높은 높이(`space.max`)가 `2500mm 초과`이면 `consult`

주의:
- 위 2개 기준(`설치공간 길이 8000 이상`, `가장 높은 높이 2500 초과`)은 **입력 차단이 아니라 상담안내 가격 처리**로 반영한다.
...
3. 타입 변경 후 기존 모듈이 설치공간 경계를 침범하면 `invalid` 또는 자동정리 정책 적용
...
- 기존 모듈 탑뷰/치수 라벨 위에 설치공간 레이어를 추가하는 방식 권장
- 설치공간 레이어와 모듈 라벨이 겹치면 우선순위 규칙 필요:
  1. 에러/경고 라벨
  2. 설치공간 라벨
  3. 모듈 라벨
...
1. 추가 위치가 설치공간 경계 내인지
2. 설치공간 허용 길이 총합을 초과하는지
...
- `타입 ㄷ자`
- `설치공간1 1800mm | 설치공간2 2400mm | 설치공간3 1800mm`
- `레이아웃 상태: 정상` 또는 `비규격 상담 안내`

### 주문완료 상세(필수)

- 타입/설치공간 사양 요약을 주문 품목 설명에 포함
...
1. 타입을 선택하면 해당 타입에 맞는 설치공간 입력 UI가 보인다.
2. 설치공간 길이 입력값 변경 시 메인 미리보기가 즉시 갱신된다.
3. 제약 초과 시 상태가 `consult` 또는 `invalid`로 명확히 표시된다.
4. 설치공간 길이 `8000mm 이상` 또는 가장 높은 높이 `2500mm 초과`일 때 상태가 `consult`로 전환된다.
5. `invalid` 상태에서는 진행/추가 액션이 차단된다.
6. 견적 상세/주문완료/메일에 타입/설치공간 정보가 포함된다.
...
4. `S-04` 검증 엔진
   - 설치공간 최소값 `460`
   - 상담안내 기준: `설치공간 길이 >= 8000`, `가장 높은 높이 > 2500`
...
- [ ] 타입 라벨 목록 확정
- [ ] 설치공간 길이 최소값 `460` / 상담기준(`>=8000`) 반영
- [ ] 가장 높은 높이 상담기준(`>2500`) 반영
- [ ] 타입 변경 시 초기화 정책 확정
- [ ] 견적/메일 표기 문구 확정
