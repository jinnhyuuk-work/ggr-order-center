...
1. 타입 선택(`ㅣ자/ㄱ자/역ㄱ자/ㄷ자/ㅁ자`) + 섹션별 사이즈 입력 + 가용범위 제약
...
주의:
- 위 2개 기준(`섹션 길이 8000 이상`, `가장 높은 높이 2500 초과`)은 **입력 차단이 아니라 상담안내 가격 처리**로 반영한다.
...
4. 섹션 길이 `8000mm 이상` 또는 가장 높은 높이 `2500mm 초과`일 때 상태가 `consult`로 전환된다.
...
4. `S-04` 검증 엔진
   - 섹션 최소값 `460`
   - 상담안내 기준: `섹션 길이 >= 8000`, `가장 높은 높이 > 2500`
